    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="48">
          ...
          <a-col :md="5" :sm="24">
            <a-form-item label="安卓版本">
              <a-select
                v-model="queryParam.androidType"
                showSearch
                placeholder="请选择版本"
          ...
          <a-col :md="5" :sm="24">
            <a-form-item label="导航类型">
              <a-select
                v-model="queryParam.navigateType"
                showSearch
                placeholder="请选择导航类型"
                optionFilterProp="children"
                :filterOption="filterOption"
                @change="searchData()"
              >
                <a-select-option value="Gs">高仙</a-select-option>
                <a-select-option value="Xl">小驴</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        <template>
          ...
          <a :href="url+'RobotVersion/fileDownload?fileName='+record.url" >下载</a>
        </template>
      </span>
    </s-table>
...
import EditForm from './modules/EditRobotApp'
import CreateForm from './modules/AddRobotApp'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return queryData(Object.assign(parameter, this.queryParam), '/RobotVersion/queryRobotVersionMsg')
          .then(res => {
            console.log(res)
            return res.result
          })
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          return new Promise((resolve, reject) => {
            ...
            queryData(param, '/RobotVersion/delRobotVersionMsg').then(res => {
