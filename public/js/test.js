...
goog.module.getInternal_ = function(a) {
    if (!COMPILED) {
        ...
        if (!goog.implicitNamespaces_[a]) return a = goog.getObjectByName(a),
        null != a ? a: null
    }
    return null
};
...
goog.require = function(a) {
    if (!COMPILED) {
        ...
        if (goog.isProvided_(a)) {
            ...
            else throw a = "goog.require could not find: " + a,
...
goog.writeScripts_ = function(a) {
    ...
    b(a);
    for (var f = 0; f < c.length; f++) a = c[f],
    goog.dependencies_.written[a] = !0;
    var g = goog.moduleLoaderState_;
    goog.moduleLoaderState_ = null;
    for (f = 0; f < c.length; f++) if (a = c[f]) {
        var h = e.loadFlags[a] || {},
...
goog.loadModule = function(a) {
    ...
    try {
        ...
        else if (goog.isString(a)) goog.useSafari10Workaround() && (a = goog.workaroundSafari10EvalBug(a)),
...
goog.userAgent.isVersionOrHigher = function(a) {
    return goog.userAgent.ASSUME_ANY_VERSION || goog.reflect.cache(goog.userAgent.isVersionOrHigherCache_, a,
    function() {
...
goog.Promise.prototype.executeCallbacks_ = function() {
    for (var a; a = this.popEntry_();) goog.Promise.LONG_STACK_TRACES && this.currentStep_++,
...
goog.events.handleBrowserEvent_ = function(a, b) {
    ...
    if (!goog.events.BrowserFeature.HAS_W3C_EVENT_SUPPORT) {
        ...
        e = new goog.events.BrowserEvent(d, this),
        f = !0;
        if (goog.events.CAPTURE_SIMULATION_MODE == goog.events.CaptureSimulationMode.ON) {
            if (!goog.events.isMarkedIeEvent_(d)) {
                goog.events.markIeEvent_(d);
                d = [];
                for (c = e.currentTarget; c; c = c.parentNode) d.push(c);
                for (var g = a.type,
                h = d.length - 1; ! e.propagationStopped_ && 0 <= h; h--) e.currentTarget = d[h],
                c = goog.events.fireListeners_(d[h], g, !0, e),
                f = f && c;
                for (h = 0; ! e.propagationStopped_ && h < d.length; h++) e.currentTarget = d[h],
                c = goog.events.fireListeners_(d[h], g, !1, e),
                f = f && c
            }
        } else f = goog.events.fireListener(a, e);
        return f
    }
...
goog.events.EventTarget.prototype.dispatchEvent = function(a) {
    ...
    if (b) {
        var c = [];
        for (var d = 1; b; b = b.getParentEventTarget()) c.push(b),
...
goog.events.EventTarget.dispatchEventInternal_ = function(a, b, c) {
    ...
    if (c) for (f = 0; ! b.propagationStopped_ && f < c.length; f++) g = b.currentTarget = c[f],
...
goog.html.TrustedResourceUrl.format = function(a, b) {
    ...
    var d = c.replace(goog.html.TrustedResourceUrl.FORMAT_MARKER_,
    function(a, d) {
...
goog.html.SAFE_URL_PATTERN_ = /^(?:(?:https?|mailto|ftp):|[^:/ ? #] * ( ? :[/?#]|$))/i; goog.html.SafeUrl.sanitize = function(a) {
    ...
    var c = b instanceof goog.html.SafeUrl ? b: goog.html.SafeUrl.sanitize(b);
...
}; goog.html.uncheckedconversions = {}; goog.html.uncheckedconversions.safeHtmlFromStringKnownToSatisfyTypeContract = function(a, b, c) {
...
}; goog.dom.$ = goog.dom.getElement; goog.dom.getElementsByTagName = function(a, b) {
    ...
    if (goog.dom.canUseQuerySelector_(a) && (e || c)) return a.querySelectorAll(e + (c ? "." + c: ""));
    if (c && a.getElementsByClassName) {
        d = a.getElementsByClassName(c);
        if (e) {
            a = {};
            for (var f = b = 0,
            g; g = d[f]; f++) e == g.nodeName && (a[b++] = g);
            a.length = b;
            return a
        }
        return d
    ...
    if (c) {
        a = {};
        for (f = b = 0; g = d[f]; f++) e = g.className,
        "function" == typeof e.split && goog.array.contains(e.split(/\s+/), c) && (a[b++] = g);
        a.length = b;
        return a
    }
    return d
...
}; goog.dom.DomHelper.prototype.$ = goog.dom.DomHelper.prototype.getElement; goog.dom.DomHelper.prototype.getElementsByTagName = function(a, b) {
    ...
    this.bottom *= c;
    return this
}; goog.math.IRect = function() {}; goog.math.Rect = function(a, b, c, d) {
    this.left = a;
    this.top = b;
    this.width = c;
    this.height = d
    ...
    var f = goog.style.getPageOffset(d),
    ...
    } else d = c.getElementsByTagNameAndClass("HEAD")[0],
    d || (e = c.getElementsByTagNameAndClass("BODY")[0], d = c.createDom("HEAD"), e.parentNode.insertBefore(d, e)),
    e = c.createDom("STYLE"),
    goog.style.setSafeStyleSheet(e, a),
    c.appendChild(d, e);
    return e
    ...
    if (goog.userAgent.IE) {
        ...
        var f = goog.style.getIePixelDistance_(a, b + "Bottom");
    ...
    if (goog.userAgent.IE && !goog.userAgent.isVersionOrHigher("7")) {
        var n = this.getAppliedCombinedClassNames_(l);
        0 < n.length && (l.push.apply(l, n), k = !0)
    }
    g && h && !m && !k || goog.dom.classlist.set(b, l.join(" "));
    return b
...
function() {
    ...
    if (a) {
        ...
        b && goog.a11y.aria.setState(a, "labelledby", b.id); (b = this.getIconElement()) && goog.a11y.aria.setRole(b, "presentation"); (b = this.getExpandIconElement()) && goog.a11y.aria.setRole(b, "presentation");
    ...
    case goog.events.KeyCodes.LEFT:
        ...
        break;
    case goog.events.KeyCodes.DOWN:
        (d = this.getNextShownNode()) && d.select();
        break;
    case goog.events.KeyCodes.UP:
        (d = this.getPreviousShownNode()) && d.select();
        break;
    default:
        b = !1
    }
    ...
    d = [], e = 0; e < a.length; e++) {
        b = a.charAt(e);
        if (!c.childNodes_[b]) throw Error('The collection does not have the key "' + a + '"');
        d.push([c, b]);
        c = c.childNodes_[b]
    }
    a = c.value_;
    for (delete c.value_; 0 < d.length;) if (b = d.pop(), c = b[0], b = b[1], c.childNodes_[b].isEmpty()) delete c.childNodes_[b];
    else break;
    return a
    ...
    var b; (b = this.typeAhead_.handleNavigation(a) || this.selectedItem_ && this.selectedItem_.onKeyDown(a) || this.typeAhead_.handleTypeAheadChar(a)) && a.preventDefault();
    return b
...
/*
 ...
 limitations under the License.
*/
Blockly.Touch = {}; Blockly.Touch.touchIdentifier_ = null; Blockly.Touch.TOUCH_MAP = {}; goog.events.BrowserFeature.TOUCH_ENABLED && (Blockly.Touch.TOUCH_MAP = {
...
Blockly.Workspace = function(a) {
    ...
    if (this.isFlyout) {
        ...
        for (var b = 0,
        c; c = a[b]; b++) this.getVariable(c) || this.createVariable(c)
    }
    ...
    for (var c = this.getAllBlocks(), e = 0; e < c.length; e++) c[e].renameVar(a.name, b),
    d && c[e].renameVar(d, b);
    ...
    var e = this,
    f = e.getVariable(a);
    1 < b.length ? Blockly.confirm(Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace("%1", b.length).replace("%2", a),
    function(a) {
    ...
    this.bubbleGroup_ = Blockly.utils.createSvgElement("g", {},
    null);
    var c = {
...
Blockly.Comment = function(a) {
    ...
    Blockly.bindEventWithChecks_(b, "wheel", this,
    function(a) {
        a.stopPropagation()
    });
    Blockly.bindEventWithChecks_(b, "change", this,
    function(a) {
    ...
    b = this.getShadowDom();
    if (a.workspace && b && Blockly.Events.recordUndo) if (a = Blockly.Xml.domToBlock(b, a.workspace), a.outputConnection) this.connect(a.outputConnection);
    ...
    if (a) if (a.outputConnection == this) var b = "Output Connection of ";
    else if (a.previousConnection == this) b = "Previous Connection of ";
    else if (a.nextConnection == this) b = "Next Connection of ";
    else if (b = goog.array.find(a.inputList,
    function(a) {
        return a.connection == this
    },
    this)) b = 'Input "' + b.name + '" connection on ';
    else return console.warn("Connection not actually connected to sourceBlock_"),
    "Orphan Connection";
    else return "Orphan Connection";
    return b + a.toDevString()
}; Blockly.Extensions = {}; Blockly.Extensions.ALL_ = {}; Blockly.Extensions.MUTATOR_PROPERTIES_ = ["domToMutation", "mutationToDom", "compose", "decompose"]; Blockly.Extensions.register = function(a, b) {
    ...
    var d, e = Blockly.Extensions.ALL_[a];
    if (!goog.isFunction(e)) throw Error('Error: Extension "' + a + '" not found.');
    c ? Blockly.Extensions.checkNoMutatorProperties_(a, b) : d = Blockly.Extensions.getMutatorProperties_(b);
    e.apply(b);
    if (c) Blockly.Extensions.checkBlockHasMutatorProperties_(a, b, 'Error after applying mutator "' + a + '": ');
    else if (!Blockly.Extensions.mutatorPropertiesMatch_(d, b)) throw Error('Error when applying extension "' + a + '": mutation properties changed when applying a non-mutator extension.');
    ...
    for (var c = 0; c < Blockly.Extensions.MUTATOR_PROPERTIES_.length; c++) {
        var d = Blockly.Extensions.MUTATOR_PROPERTIES_[c];
        if (b.hasOwnProperty(d)) throw Error('Error: tried to apply mutation "' + a + '" to a block that already has a "' + d + '" function.  Block id: ' + b.id);
    }
}; Blockly.Extensions.checkMutatorDialog_ = function(a, b) {
    var c = a.hasOwnProperty("compose"),
    d = a.hasOwnProperty("decompose");
    if (c && d) {
    ...
    if (!b.hasOwnProperty("domToMutation")) throw Error(a + 'Applying a mutator didn\'t add "domToMutation"');
    if (!b.hasOwnProperty("mutationToDom")) throw Error(a + 'Applying a mutator didn\'t add "mutationToDom"');
    Blockly.Extensions.checkMutatorDialog_(b, a)
}; Blockly.Extensions.getMutatorProperties_ = function(a) {
    for (var b = [], c = 0; c < Blockly.Extensions.MUTATOR_PROPERTIES_.length; c++) b.push(a[Blockly.Extensions.MUTATOR_PROPERTIES_[c]]);
    return b
    ...
    else for (var e = 0; e < d.length; e++) a[e] != d[e] && (c = !1);
    return c
    ...
    var d = a.getField(b);
    if (!d.isOptionListDynamic()) for (var d = d.getOptions(), e = 0; e < d.length; ++e) {
        var f = d[e][1];
...
}; Blockly.Extensions.register("parent_tooltip_when_inline", Blockly.Extensions.extensionParentTooltip_); Blockly.RenderedConnection = function(a, b) {
    Blockly.RenderedConnection.superClass_.constructor.call(this, a, b);
    this.offsetInBlock_ = new goog.math.Coordinate(0, 0)
}; goog.inherits(Blockly.RenderedConnection, Blockly.Connection); Blockly.RenderedConnection.prototype.distanceFrom = function(a) {
    var b = this.x_ - a.x_;
    a = this.y_ - a.y_;
...
}; Blockly.RenderedConnection.prototype.setHidden = function(a) { (this.hidden_ = a) && this.inDB_ ? this.db_.removeConnection_(this) : a || this.inDB_ || this.db_.addConnection(this)
    ...
    this.closestConnection_ && (this.localConnection_.connect(this.closestConnection_), this.topBlock_.rendered && (this.localConnection_.isSuperior() ? this.closestConnection_: this.localConnection_).getSourceBlock().connectionUiEffect(), this.removeHighlighting_())
    ...
    this.wouldDeleteBlock_ = !!b && !this.topBlock_.getParent() && this.topBlock_.isDeletable() && !c; ! this.wouldDeleteBlock_ && d && this.closestConnection_ && this.addHighlighting_()
...
}; Blockly.constants = {}; Blockly.DRAG_RADIUS = 5; Blockly.FLYOUT_DRAG_RADIUS = 10; Blockly.SNAP_RADIUS = 20; Blockly.BUMP_DELAY = 250; Blockly.COLLAPSE_CHARS = 30; Blockly.LONGPRESS = 750; Blockly.SOUND_LIMIT = 100; Blockly.DRAG_STACK = !0; Blockly.HSV_SATURATION = .45; Blockly.HSV_VALUE = .65; Blockly.SPRITE = {
    width: 96,
    height: 124,
    url: "sprites.png"
}; Blockly.SVG_NS = "http://www.w3.org/2000/svg"; Blockly.HTML_NS = "http://www.w3.org/1999/xhtml"; Blockly.INPUT_VALUE = 1; Blockly.OUTPUT_VALUE = 2; Blockly.NEXT_STATEMENT = 3; Blockly.PREVIOUS_STATEMENT = 4; Blockly.DUMMY_INPUT = 5; Blockly.ALIGN_LEFT = -1; Blockly.ALIGN_CENTRE = 0; Blockly.ALIGN_RIGHT = 1; Blockly.DRAG_NONE = 0; Blockly.DRAG_STICKY = 1; Blockly.DRAG_BEGIN = 1; Blockly.DRAG_FREE = 2; Blockly.OPPOSITE_TYPE = []; Blockly.OPPOSITE_TYPE[Blockly.INPUT_VALUE] = Blockly.OUTPUT_VALUE; Blockly.OPPOSITE_TYPE[Blockly.OUTPUT_VALUE] = Blockly.INPUT_VALUE; Blockly.OPPOSITE_TYPE[Blockly.NEXT_STATEMENT] = Blockly.PREVIOUS_STATEMENT; Blockly.OPPOSITE_TYPE[Blockly.PREVIOUS_STATEMENT] = Blockly.NEXT_STATEMENT; Blockly.TOOLBOX_AT_TOP = 0; Blockly.TOOLBOX_AT_BOTTOM = 1; Blockly.TOOLBOX_AT_LEFT = 2; Blockly.TOOLBOX_AT_RIGHT = 3; Blockly.DELETE_AREA_NONE = null; Blockly.DELETE_AREA_TRASH = 1; Blockly.DELETE_AREA_TOOLBOX = 2; Blockly.VARIABLE_CATEGORY_NAME = "VARIABLE"; Blockly.PROCEDURE_CATEGORY_NAME = "PROCEDURE"; Blockly.RENAME_VARIABLE_ID = "RENAME_VARIABLE_ID"; Blockly.DELETE_VARIABLE_ID = "DELETE_VARIABLE_ID"; Blockly.WorkspaceDragger = function(a) {
    this.workspace_ = a;
    ...
    if (this.flyout_ ? this.flyout_.isScrollable() : this.startWorkspace_ && this.startWorkspace_.isDraggable()) this.workspaceDragger_ = this.flyout_ ? new Blockly.FlyoutDragger(this.flyout_) : new Blockly.WorkspaceDragger(this.startWorkspace_),
    ...
    this.fieldGroup_ || (this.fieldGroup_ = Blockly.utils.createSvgElement("g", {},
    null), this.visible_ || (this.fieldGroup_.style.display = "none"), this.borderRect_ = Blockly.utils.createSvgElement("rect", {
        rx: 4,
        ry: 4,
    ...
    null !== a && (a = String(a), a !== this.text_ && (this.text_ = a, this.size_.width = 0, this.sourceBlock_ && this.sourceBlock_.rendered && (this.sourceBlock_.render(), this.sourceBlock_.bumpNeighbours_())))
    ...
    c = null;
    for (var g = b,
    h, k = f - 1; 0 <= k && this.isInYRange_(k, a.y_, b);) h = this[k],
    a.isConnectionAllowed(h, g) && (c = h, g = h.distanceFrom(a)),
    k--;
    for (; f < this.length && this.isInYRange_(f, a.y_, b);) h = this[f],
    a.isConnectionAllowed(h, g) && (c = h, g = h.distanceFrom(a)),
    f++;
    a.x_ = e;
    a.y_ = d;
...
}; Blockly.utils = {}; Blockly.utils.removeAttribute = function(a, b) {
    ...
    if (c = (c = a.getAttribute("transform")) && c.match(Blockly.utils.getRelativeXY.XY_REGEX_)) b.x += parseFloat(c[1]),
    c[3] && (b.y += parseFloat(c[3])); (a = a.getAttribute("style")) && -1 < a.indexOf("translate") && ((c = a.match(Blockly.utils.getRelativeXY.XY_2D_REGEX_)) || (c = a.match(Blockly.utils.getRelativeXY.XY_3D_REGEX_)), c && (b.x += parseFloat(c[1]), c[3] && (b.y += parseFloat(c[3]))));
    return b
    ...
    for (var c = 0,
    d = b || Blockly.utils.shortestStringLength(a), e = 0; e < d; e++) {
        var f = a[0][e];
        for (var g = 1; g < a.length; g++) if (f != a[g][e]) return c;
        " " == f && (c = e + 1)
    }
    ...
    for (var c = 0,
    d = b || Blockly.utils.shortestStringLength(a), e = 0; e < d; e++) {
        var f = a[0].substr( - e - 1, 1);
        for (var g = 1; g < a.length; g++) if (f != a[g].substr( - e - 1, 1)) return c;
        " " == f && (c = e + 1)
    }
    ...
    for (var b = !0,
    c = /%{BKY_([a-zA-Z][a-zA-Z0-9_]*)}/g,
    d = c.exec(a); null != d;) {
        var e = d[1];
        null == Blockly.Msg[e] && (console.log("WARNING: No message string for %{BKY_" + e + "}."), b = !1);
    ...
    d.push("");
    for (var e = 0,
    f = [], g = null, h = 0; h < d.length; h++) {
        var k = d[h];
        0 == e ? "%" == k ? ((k = f.join("")) && c.push(k), f.length = 0, e = 1) : f.push(k) : 1 == e ? "%" == k ? (f.push(k), e = 0) : b && "0" <= k && "9" >= k ? (e = 2, g = k, (k = f.join("")) && c.push(k), f.length = 0) : "{" == k ? e = 3 : (f.push("%", k), e = 0) : 2 == e ? "0" <= k && "9" >= k ? g += k: (c.push(parseInt(g, 10)), h--, e = 0) : 3 == e && ("" == k ? (f.splice(0, 0, "%{"), h--, e = 0) : "}" != k ? f.push(k) : (k = f.join(""), /[a-zA-Z][a-zA-Z0-9_]*/.test(k) ? (e = k.toUpperCase(), (e = goog.string.startsWith(e, "BKY_") ? e.substring(4) : null) && e in Blockly.Msg ? (k = Blockly.Msg[e], goog.isString(k) ? Array.prototype.push.apply(c, Blockly.utils.tokenizeInterpolation(k)) : b ? c.push(String(k)) : c.push(k)) : c.push("%{" + k + "}")) : c.push("%{" + k + "}"), e = f.length = 0))
    } (k = f.join("")) && c.push(k);
    d = [];
    for (h = f.length = 0; h < c.length; ++h)"string" == typeof c[h] ? f.push(c[h]) : ((k = f.join("")) && d.push(k), f.length = 0, d.push(c[h])); (k = f.join("")) && d.push(k);
    f.length = 0;
    return d
}; Blockly.utils.genUid = function() {
    for (var a = Blockly.utils.genUid.soup_.length,
    b = [], c = 0; 20 > c; c++) b[c] = Blockly.utils.genUid.soup_.charAt(Math.random() * a);
    return b.join("")
}; Blockly.utils.genUid.soup_ = "!#$%()*+,-./:;=?@[]^_`{|}~ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"; Blockly.utils.wrap = function(a, b) {
    ...
    do {
        var f = d;
        var g = a;
        for (var h = [], k = c.length / e, l = 1, d = 0; d < c.length - 1; d++) l < (d + 1.5) / k ? (l++, h[d] = !0) : h[d] = !1;
    ...
    for (var d in c) if (void 0 !== a.style[d]) {
        ...
        if (!b) return document.body.removeChild(a),
        !1;
...
}; Blockly.Grid.prototype.scale_ = 1; Blockly.Grid.prototype.dispose = function() {
    ...
    var b = Math.floor(this.spacing_ / 2) + .5,
    c = b - this.length_ / 2,
    d = b + this.length_ / 2,
    b = b * a,
    c = c * a,
    ...
    this.gridPattern_.setAttribute("y", b); (goog.userAgent.IE || goog.userAgent.EDGE) && this.update(this.scale_)
    ...
    var b, c, d, e, f, g = !!a.readOnly;
    if (g) {
        var h = null;
        var k = b = c = d = e = f = !1
    } else h = Blockly.Options.parseToolboxTree(a.toolbox),
    f = !(!h || !h.getElementsByTagName("category").length),
    e = a.trashcan,
    void 0 === e && (e = f),
    d = a.collapse,
    void 0 === d && (d = f),
    ...
    void 0 === m && (m = !1);
    var n = a.toolboxPosition;
    n = "end" === n ? !1 : !0;
    var p = m ? n ? Blockly.TOOLBOX_AT_TOP: Blockly.TOOLBOX_AT_BOTTOM: n == l ? Blockly.TOOLBOX_AT_RIGHT: Blockly.TOOLBOX_AT_LEFT;
    var q = a.scrollbars;
    void 0 === q && (q = f);
    var t = a.css;
    void 0 === t && (t = !0);
    ...
    n = void 0 === a.oneBasedIndex ? !0 : !!a.oneBasedIndex;
    this.RTL = l;
    this.oneBasedIndex = n;
    this.collapse = d;
    this.comments = c;
    this.disable = b;
    this.readOnly = g;
...
}; Blockly.Options.prototype.parentWorkspace = null; Blockly.Options.prototype.setMetrics = null; Blockly.Options.prototype.getMetrics = null; Blockly.Options.parseZoomOptions_ = function(a) {
    a = a.zoom || {};
    var b = {};
...
}; Blockly.WorkspaceAudio.prototype.lastSound_ = null; Blockly.WorkspaceAudio.prototype.dispose = function() {
    ...
    this.SVG_.style.display = "block"
}; Blockly.Xml = {}; Blockly.Xml.workspaceToDom = function(a, b) {
    ...
    var c, d = goog.dom.createDom(a.isShadow() ? "shadow": "block");
    d.setAttribute("type", a.type);
    b || d.setAttribute("id", a.id);
    if (a.mutationToDom) {
        var e = a.mutationToDom();
        e && (e.hasChildNodes() || e.hasAttributes()) && d.appendChild(e)
    }
    for (var e = 0,
    f; f = a.inputList[e]; e++) {
        var g = 0;
        for (var h; h = f.fieldRow[g]; g++) {
            var k = h;
            k.name && k.EDITABLE && (h = goog.dom.createDom("field", null, k.getValue()), h.setAttribute("name", k.name), k instanceof Blockly.FieldVariable && (k = a.workspace.getVariable(k.getValue())) && (h.setAttribute("id", k.getId()), h.setAttribute("variableType", k.type)), d.appendChild(h))
        }
    }
    if (e = a.getCommentText()) e = goog.dom.createDom("comment", null, e),
    "object" == typeof a.comment && (e.setAttribute("pinned", a.comment.isVisible()), f = a.comment.getBubbleSize(), e.setAttribute("h", f.height), e.setAttribute("w", f.width)),
    d.appendChild(e);
    a.data && (e = goog.dom.createDom("data", null, a.data), d.appendChild(e));
    for (e = 0; f = a.inputList[e]; e++) h = !0,
    f.type != Blockly.DUMMY_INPUT && (k = f.connection.targetBlock(), f.type == Blockly.INPUT_VALUE ? c = goog.dom.createDom("value") : f.type == Blockly.NEXT_STATEMENT && (c = goog.dom.createDom("statement")), g = f.connection.getShadowDom(), !g || k && k.isShadow() || c.appendChild(Blockly.Xml.cloneShadow_(g)), k && (c.appendChild(Blockly.Xml.blockToDom(k, b)), h = !1), c.setAttribute("name", f.name), h || d.appendChild(c));
    a.inputsInlineDefault != a.inputsInline && d.setAttribute("inline", a.inputsInline);
    a.isCollapsed() && d.setAttribute("collapsed", !0);
    a.disabled && d.setAttribute("disabled", !0);
    a.isDeletable() || a.isShadow() || d.setAttribute("deletable", !1);
    a.isMovable() || a.isShadow() || d.setAttribute("movable", !1);
    a.isEditable() || d.setAttribute("editable", !1);
    if (e = a.getNextBlock()) c = goog.dom.createDom("next", null, Blockly.Xml.blockToDom(e, b)),
    d.appendChild(c);
    g = a.nextConnection && a.nextConnection.getShadowDom(); ! g || e && e.isShadow() || c.appendChild(Blockly.Xml.cloneShadow_(g));
    return d
    ...
    a = a.replace(/(<(\w+)\b[^>]*>[^\n]*)\n *<\/\2>/g, "$1</$2>");
    return a.replace(/^\n/, "")
}; Blockly.Xml.textToDom = function(a) { (a = (new DOMParser).parseFromString(a, "text/xml")) && a.firstChild && "xml" == a.firstChild.nodeName.toLowerCase() && a.firstChild === a.lastChild || goog.asserts.fail("Blockly.Xml.textToDom did not obtain a valid XML tree.");
    return a.firstChild
    ...
    try {
        for (var h = 0; h < e; h++) {
            ...
            if ("block" == l || "shadow" == l && !Blockly.Events.recordUndo) {
                var m = Blockly.Xml.domToBlock(k, b);
                c.push(m.id);
                var n = parseInt(k.getAttribute("x"), 10),
                p = parseInt(k.getAttribute("y"), 10);
                isNaN(n) || isNaN(p) || m.moveBy(b.RTL ? d - n: n, p);
                g = !1
            } else if ("shadow" == l) goog.asserts.fail("Shadow block cannot be a top-level block."),
            g = !1;
            else if ("variables" == l) {
    ...
    for (var e = a.getAttribute("id"), c = b.newBlock(d, e), f = null, g = 0, h; h = a.childNodes[g]; g++) if (3 != h.nodeType) {
        for (var k = null,
        l = null,
        f = 0,
        m; m = h.childNodes[f]; f++) 1 == m.nodeType && ("block" == m.nodeName.toLowerCase() ? k = m: "shadow" == m.nodeName.toLowerCase() && (l = m)); ! k && l && (k = l);
        ...
        case "comment":
            ...
            h = parseInt(h.getAttribute("h"), 10); ! isNaN(f) && !isNaN(h) && c.comment && c.comment.setVisible && c.comment.setBubbleSize(f, h);
            break;
        case "data":
        ...
        case "field":
            ...
            if (k instanceof Blockly.FieldVariable) {
                m = h.getAttribute("variableType") || "";
                var p = b.getVariable(l);
                p || (p = b.createVariable(l, m, h.getAttribute(e)));
                if (null !== m && m !== p.type) throw Error("Serialized variable type with id '" + p.getId() + "' had type " + p.type + ", and does not match variable field that references it: " + Blockly.Xml.domToText(h) + ".");
            }
            if (!k) {
        ...
        case "statement":
            ...
            break;
        case "next":
            l && c.nextConnection && c.nextConnection.setShadowDom(l);
    ...
    } (g = a.getAttribute("inline")) && c.setInputsInline("true" == g); (g = a.getAttribute("disabled")) && c.setDisabled("true" == g); (g = a.getAttribute("deletable")) && c.setDeletable("true" == g); (g = a.getAttribute("movable")) && c.setMovable("true" == g); (g = a.getAttribute("editable")) && c.setEditable("true" == g); (g = a.getAttribute("collapsed")) && c.setCollapsed("true" == g);
...
Blockly.ZoomControls = function(a) {
    ...
    Blockly.bindEventWithChecks_(c, "mousedown", null,
    function(b) {
        a.markFocused();
    ...
    Blockly.bindEventWithChecks_(e, "mousedown", null,
    function(b) {
        a.markFocused();
        a.zoomCenter(1);
        Blockly.Touch.clearTouchIdentifier();
    ...
    Blockly.bindEventWithChecks_(d, "mousedown", null,
    function(b) {
        a.markFocused();
        a.zoomCenter( - 1);
        Blockly.Touch.clearTouchIdentifier();
...
Blockly.WorkspaceSvg = function(a, b, c) {
    ...
    if (void 0 === b) {
        for (var c = 0,
        d; d = this.highlightedBlocks_[c]; c++) d.setHighlighted(!1);
        this.highlightedBlocks_.length = 0
    }
    if (d = a ? this.getBlockById(a) : null)(c = void 0 === b || b) ? -1 == this.highlightedBlocks_.indexOf(d) && this.highlightedBlocks_.push(d) : goog.array.remove(this.highlightedBlocks_, d),
    d.setHighlighted(c)
    ...
    if (this.rendered && !(a.getElementsByTagName("block").length >= this.remainingCapacity())) {
        ...
        try {
            ...
            if (goog.userAgent.IE || goog.userAgent.EDGE) for (var c = b.getDescendants(), d = c.length - 1; 0 <= d; d--) c[d].render(!1);
            ...
            if (!isNaN(e) && !isNaN(f)) {
                ...
                do {
                    var g = !1;
                    for (var h = this.getAllBlocks(), d = 0, k; k = h[d]; d++) {
                        var l = k.getRelativeToSurfaceXY();
                        if (1 >= Math.abs(e - l.x) && 1 >= Math.abs(f - l.y)) {
                            g = !0;
                            break
                        }
                    }
                    if (!g) for (var m = b.getConnections_(!1), d = 0, n; n = m[d]; d++) if (n.closest(Blockly.SNAP_RADIUS, new goog.math.Coordinate(e, f)).connection) {
                        g = !0;
                        break
                    }
                    g && (e = this.RTL ? e - Blockly.SNAP_RADIUS: e + Blockly.SNAP_RADIUS, f += 2 * Blockly.SNAP_RADIUS)
                } while ( g );
                b.moveBy(e, f)
            }
        } finally {
    ...
    if (!this.options.readOnly && !this.isFlyout) {
        ...
        d.push(h);
        this.scrollbar && (h = {},
        h.text = Blockly.Msg.CLEAN_UP, h.enabled = 1 < e.length, h.callback = this.cleanUp.bind(this), d.push(h));
        if (this.options.collapse) {
            for (var k = h = !1,
            l = 0; l < e.length; l++) for (var m = e[l]; m;) m.isCollapsed() ? h = !0 : k = !0,
            m = m.getNextBlock();
            var n = function(a) {
                for (var b = 0,
        ...
        for (var p = [], l = 0; l < e.length; l++) b(e[l]);
        h = {
            ...
            callback: function() {
                ...
                2 > p.length ? c() : Blockly.confirm(Blockly.Msg.DELETE_ALL_BLOCKS.replace("%1", p.length),
                function(a) {
                    a && c()
                })
            }
    ...
    if (this.toolbox_) if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP || this.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM) a.height -= this.toolbox_.getHeight();
    ...
    var c = a.width - b;
    var d = a.height - b;
    var e = this.getBlocksBoundingBox();
    var f = e.width * this.scale;
    var g = e.height * this.scale,
    h = e.x * this.scale,
    k = e.y * this.scale;
    this.scrollbar ? (b = Math.min(h - c / 2, h + f - c), f = Math.max(h + f + c / 2, h + c), c = Math.min(k - d / 2, k + g - d), d = Math.max(k + g + d / 2, k + d)) : (b = e.x, f = b + e.width, c = e.y, d = c + e.height);
    e = 0;
    ...
    if (this.quarkNames_.length) {
        var a = goog.dom.createDom("xml");
        for (var b = 0,
        c; c = this.quarkNames_[b]; b++) a.appendChild(goog.dom.createDom("block", {
            type: c
        }))
    } else a = null;
    a = {
    ...
    if (a != this.isVisible()) if (Blockly.Events.fire(new Blockly.Events.Ui(this.block_, "mutatorOpen", !a, a)), a) {
        ...
        var b = 0;
        for (var c; c = a[b]; b++) c.render();
        this.rootBlock_.setMovable(!1);
        this.rootBlock_.setDeletable(!1);
        this.workspace_.flyout_ ? (b = 2 * this.workspace_.flyout_.CORNER_RADIUS, a = this.workspace_.flyout_.width_ + b) : a = b = 16;
        this.block_.RTL && (a = -a);
    ...
    var a;
    if (!this.workspace_.isDragging()) for (var b = this.workspace_.getTopBlocks(!1), c = 0; a = b[c]; c++) {
        var d = a.getRelativeToSurfaceXY(),
        e = a.getHeightWidth();
        20 > d.y + e.height && a.moveBy(0, 20 - e.height - d.y)
    }
    if (this.rootBlock_.workspace == this.workspace_) {
        Blockly.Events.setGroup(!0);
        a = this.block_;
        b = (b = a.mutationToDom()) && Blockly.Xml.domToText(b);
        c = a.rendered;
        a.rendered = !1;
        a.compose(this.rootBlock_);
        a.rendered = c;
        a.initSvg();
        c = (c = a.mutationToDom()) && Blockly.Xml.domToText(c);
        if (b != c) {
            Blockly.Events.fire(new Blockly.Events.BlockChange(a, "mutation", null, b, c));
    ...
    if (this.workspace) {
        ...
        try {
            ...
            for (var b = 0,
            c; c = this.inputList[b]; b++) c.dispose();
    ...
    var b;
    if (!a && this.isCollapsed()) this.outputConnection && this.outputConnection.setHidden(a),
    this.previousConnection && this.previousConnection.setHidden(a),
    this.nextConnection && (this.nextConnection.setHidden(a), (b = this.nextConnection.targetBlock()) && b.setConnectionsHidden(a));
    ...
    this.onchangeWrapper_ && this.workspace.removeChangeListener(this.onchangeWrapper_);
    if (this.onchange = a) this.onchangeWrapper_ = a.bind(this),
    ...
    f; f = this.inputList[e]; e++) {
        for (var g = 0,
        h; h = f.fieldRow[g]; g++) h instanceof Blockly.FieldDropdown && !h.getValue() ? c.push(d) : c.push(h.getText());
    ...
    for (b = 0; void 0 !== a["message" + b];) this.interpolate_(a["message" + b], a["args" + b] || [], a["lastDummyAlign" + b]),
    b++;
    ...
    b = [];
    for (g = 0; g < a.length; g++) if (f = a[g], "string" == typeof f) b.push([f, void 0]);
    else {
        d = h = null;
        do
        if (e = !1, "string" == typeof f) h = new Blockly.FieldLabel(f);
        else switch (f.type) {
        case "input_value":
            d = this.appendValueInput(f.name);
            break;
        case "input_statement":
            d = this.appendStatementInput(f.name);
            break;
        case "input_dummy":
            d = this.appendDummyInput(f.name);
            break;
        case "field_label":
            h = Blockly.Block.newFieldLabelFromJson_(f);
            break;
        case "field_input":
            h = Blockly.Block.newFieldTextInputFromJson_(f);
            break;
        case "field_angle":
            h = new Blockly.FieldAngle(f.angle);
            break;
        case "field_checkbox":
            h = new Blockly.FieldCheckbox(f.checked ? "TRUE": "FALSE");
            break;
        case "field_colour":
            h = new Blockly.FieldColour(f.colour);
            break;
        case "field_variable":
            h = Blockly.Block.newFieldVariableFromJson_(f);
            break;
        case "field_dropdown":
            h = new Blockly.FieldDropdown(f.options);
            break;
        case "field_image":
            h = Blockly.Block.newFieldImageFromJson_(f);
            break;
        case "field_number":
        ...
        case "field_date":
            if (Blockly.FieldDate) {
                h = new Blockly.FieldDate(f.date);
                break
            }
        default:
            f.alt && (f = f.alt, e = !0)
        ...
        while (e);
        if (h) b.push([h, f.name]);
        else if (d) {
            f.check && d.setCheck(f.check);
            f.align && d.setAlign(c[f.align]);
    ...
    for (var b = 0,
    c; c = this.inputList[b]; b++) if (c.connection && (c = c.connection.targetBlock(), !c || !c.allInputsFilled(a))) return ! 1;
    ...
    this.container_ = a;
    this.createDom()
}; Blockly.BlockDragSurfaceSvg.prototype.SVG_ = null; Blockly.BlockDragSurfaceSvg.prototype.dragGroup_ = null; Blockly.BlockDragSurfaceSvg.prototype.container_ = null; Blockly.BlockDragSurfaceSvg.prototype.scale_ = 1; Blockly.BlockDragSurfaceSvg.prototype.surfaceXY_ = null; Blockly.BlockDragSurfaceSvg.prototype.createDom = function() {
    this.SVG_ || (this.SVG_ = Blockly.utils.createSvgElement("svg", {
        xmlns: Blockly.SVG_NS,
        "xmlns:html": Blockly.HTML_NS,
        "xmlns:xlink": "http://www.w3.org/1999/xlink",
    ...
    this.container_), this.dragGroup_ = Blockly.utils.createSvgElement("g", {},
    this.SVG_))
...
}; Blockly.ContextMenu = {}; Blockly.ContextMenu.currentBlock = null; Blockly.ContextMenu.eventWrapper_ = null; Blockly.ContextMenu.show = function(a, b, c) {
    ...
    a = a.getElement();
    Blockly.utils.addClass(a, "blocklyContextMenu");
    Blockly.bindEventWithChecks_(a, "contextmenu", null, Blockly.utils.noEvent);
    a = goog.style.getSize(a);
    var f = b.clientX + e.x,
    g = b.clientY + e.y;
    b.clientY + a.height >= d.height && (g -= a.height);
    c ? a.width >= b.clientX && (f += a.width) : b.clientX + a.width >= d.width && (f -= a.width);
    Blockly.WidgetDiv.position(f, g, d, e, c)
    ...
    if (d) {
        do {
            var e = Blockly.utils.getRelativeXY(d), a = a + e.x, b = b + e.y;
    ...
    } else d = new goog.math.Coordinate(a.x - b, a.y),
    ...
    } else Blockly.draggingConnections_ = [],
    ...
    e; e = a[d]; d++) {
        var f = e.getSvgRoot();
        f && ((goog.userAgent.IE || goog.userAgent.EDGE) && e.updateWidth(), this.RTL ? (b -= e.renderSep + e.renderWidth, f.setAttribute("transform", "translate(" + b + "," + c + ")"), e.renderWidth && (b -= Blockly.BlockSvg.SEP_SPACE_X)) : (f.setAttribute("transform", "translate(" + (b + e.renderSep) + "," + c + ")"), e.renderWidth && (b += e.renderSep + e.renderWidth + Blockly.BlockSvg.SEP_SPACE_X)))
    }
    return this.RTL ? -b: b
    ...
    h = !1,
    k = void 0,
    l = this.getInputsInline() && !this.isCollapsed(), m = 0, n; n = b[m]; m++) if (n.isVisible()) {
        if (l && k && k != Blockly.NEXT_STATEMENT && n.type != Blockly.NEXT_STATEMENT) var p = c[c.length - 1];
        else k = n.type,
        p = [],
        p.type = l && n.type != Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE: n.type,
        p.height = 0,
        c.push(p);
        p.push(n);
        n.renderHeight = Blockly.BlockSvg.MIN_BLOCK_Y;
        n.renderWidth = l && n.type == Blockly.INPUT_VALUE ? Blockly.BlockSvg.TAB_WIDTH + 1.25 * Blockly.BlockSvg.SEP_SPACE_X: 0;
        if (n.connection && n.connection.isConnected()) {
            var q = n.connection.targetBlock().getHeightWidth();
            n.renderHeight = Math.max(n.renderHeight, q.height);
            n.renderWidth = Math.max(n.renderWidth, q.width)
        }
        l || m != b.length - 1 ? !l && n.type == Blockly.INPUT_VALUE && b[m + 1] && b[m + 1].type == Blockly.NEXT_STATEMENT && n.renderHeight--:n.renderHeight--;
        p.height = Math.max(p.height, n.renderHeight);
        n.fieldWidth = 0;
        1 == c.length && (n.fieldWidth += this.RTL ? -a: a);
        for (var q = !1,
        t = 0,
        r; r = n.fieldRow[t]; t++) {
            0 != t && (n.fieldWidth += Blockly.BlockSvg.SEP_SPACE_X);
            var u = r.getSize();
            r.renderWidth = u.width;
            r.renderSep = q && r.EDITABLE ? Blockly.BlockSvg.SEP_SPACE_X: 0;
            n.fieldWidth += r.renderWidth + r.renderSep;
            p.height = Math.max(p.height, u.height);
            q = r.EDITABLE
        }
        p.type != Blockly.BlockSvg.INLINE && (p.type == Blockly.NEXT_STATEMENT ? (g = !0, e = Math.max(e, n.fieldWidth)) : (p.type == Blockly.INPUT_VALUE ? f = !0 : p.type == Blockly.DUMMY_INPUT && (h = !0), d = Math.max(d, n.fieldWidth)))
    }
    for (a = 0; p = c[a]; a++) if (p.thicker = !1, p.type == Blockly.BlockSvg.INLINE) for (b = 0; n = p[b]; b++) if (n.type == Blockly.INPUT_VALUE) {
    ...
    q; q = e[p]; p++) {
        ...
        0 == p && (h += this.RTL ? -f: f);
        b.push("M", e.rightEdge - .5 + "," + (m + .5));
        if (this.isCollapsed()) l = q[0],
        k = h,
        h = m,
        this.renderFields_(l.fieldRow, k, h),
        a.push(Blockly.BlockSvg.JAGGED_TEETH),
        b.push("h 8"),
        l = q.height - Blockly.BlockSvg.JAGGED_TEETH_HEIGHT,
        a.push("v", l),
    ...
    return ! 1
}; Blockly.Events.Abstract.prototype.run = function(a) {}; Blockly.Events.Abstract.prototype.getEventWorkspace_ = function() {
...
}; goog.inherits(Blockly.Events.Create, Blockly.Events.Abstract); Blockly.Events.BlockCreate = Blockly.Events.Create; Blockly.Events.Create.prototype.type = Blockly.Events.CREATE; Blockly.Events.Create.prototype.toJson = function() {
...
}; goog.inherits(Blockly.Events.Delete, Blockly.Events.Abstract); Blockly.Events.BlockDelete = Blockly.Events.Delete; Blockly.Events.Delete.prototype.type = Blockly.Events.DELETE; Blockly.Events.Delete.prototype.toJson = function() {
...
}; goog.inherits(Blockly.Events.Change, Blockly.Events.Abstract); Blockly.Events.BlockChange = Blockly.Events.Change; Blockly.Events.Change.prototype.type = Blockly.Events.CHANGE; Blockly.Events.Change.prototype.toJson = function() {
...
}; goog.inherits(Blockly.Events.Move, Blockly.Events.Abstract); Blockly.Events.BlockMove = Blockly.Events.Move; Blockly.Events.Move.prototype.type = Blockly.Events.MOVE; Blockly.Events.Move.prototype.toJson = function() {
...
}; goog.inherits(Blockly.Events.VarCreate, Blockly.Events.Abstract); Blockly.Events.VarCreate.prototype.type = Blockly.Events.VAR_CREATE; Blockly.Events.VarCreate.prototype.toJson = function() {
...
}; goog.inherits(Blockly.Events.VarDelete, Blockly.Events.Abstract); Blockly.Events.VarDelete.prototype.type = Blockly.Events.VAR_DELETE; Blockly.Events.VarDelete.prototype.toJson = function() {
...
}; goog.inherits(Blockly.Events.VarRename, Blockly.Events.Abstract); Blockly.Events.VarRename.prototype.type = Blockly.Events.VAR_RENAME; Blockly.Events.VarRename.prototype.toJson = function() {
    ...
    return function() {
        var b = Blockly.FieldTextInput.htmlInput_,
        c = b.value;
        if (a.sourceBlock_) if (c = a.callValidator(c), null === c) c = b.defaultValue;
    ...
    this.symbol_ = Blockly.utils.createSvgElement("tspan", {},
    null);
    ...
    var a = Blockly.WidgetDiv.DIV;
    if (a.firstChild) {
        var a = Blockly.utils.createSvgElement("svg", {
        ...
        a),
        b = Blockly.utils.createSvgElement("circle", {
    ...
    if (!this.fieldGroup_) {
        this.arrow_ = Blockly.utils.createSvgElement("tspan", {},
        null);
    ...
    f.y = f.y + h.height + e.height >= c.height + d.y ? f.y - (h.height + 2) : f.y + e.height;
    this.sourceBlock_.RTL ? (f.x += e.width, f.x += Blockly.FieldDropdown.CHECKMARK_OVERHANG, f.x < d.x + h.width && (f.x = d.x + h.width)) : (f.x -= Blockly.FieldDropdown.CHECKMARK_OVERHANG, f.x > c.width + d.x - h.width && (f.x = c.width + d.x - h.width));
    ...
    if (goog.isArray(a)) {
        ...
        if (! (b || 2 > a.length)) {
            for (var e = [], c = 0; c < a.length; c++) e.push(a[c][0]);
            c = Blockly.utils.shortestStringLength(e);
            b = Blockly.utils.commonWordPrefix(e, c);
            d = Blockly.utils.commonWordSuffix(e, c);
            if ((b || d) && !(c <= b + d)) {
                b && (this.prefixField = e[0].substring(0, b - 1));
                d && (this.suffixField = e[0].substr(1 - d));
                e = [];
                for (c = 0; c < a.length; c++) {
                    var f = a[c][0],
                    g = a[c][1],
                    f = f.substring(b, f.length - d);
                    e[c] = [f, g]
                }
    ...
    if (null !== a && a !== this.value_) {
        ...
        for (var b = this.getOptions(), c = 0; c < b.length; c++) if (b[c][1] == a) {
            a = b[c][0];
            "object" == typeof a ? (this.imageJson_ = a, this.setText(a.alt)) : (this.imageJson_ = null, this.setText(a));
            return
        }
        this.setText(a)
    }
    ...
    if (this.visible_) {
        ...
        } else a = document.createTextNode(this.getDisplayText_()),
        this.textElement_.appendChild(a),
    ...
    this.fieldGroup_ || (this.fieldGroup_ = Blockly.utils.createSvgElement("g", {},
    null), this.visible_ || (this.fieldGroup_.style.display = "none"), this.imageElement_ = Blockly.utils.createSvgElement("image", {
    ...
    null !== a && (this.text_ = a)
}; Blockly.FieldImage.prototype.render_ = function() {}; Blockly.FieldImage.prototype.updateWidth = function() {}; Blockly.FieldImage.prototype.showEditor = function() {
...
}; goog.inherits(Blockly.FieldNumber, Blockly.FieldTextInput); Blockly.FieldNumber.prototype.setConstraints = function(a, b, c) {
    c = parseFloat(c);
...
}; Blockly.Variables = {}; Blockly.Variables.NAME_TYPE = Blockly.VARIABLE_CATEGORY_NAME; Blockly.Variables.allUsedVariables = function(a) {
    ...
    a.registerButtonCallback("CREATE_VARIABLE",
    function(a) {
    ...
    e = "ijkmnopqrstuvwxyzabcdefgh".charAt(d); ! b;) {
        for (var f = !1,
        g = 0; g < a.length; g++) if (a[g].name.toLowerCase() == e) {
            f = !0;
            break
        }
    ...
    var d = function(e) {
        Blockly.Variables.promptName(Blockly.Msg.NEW_VARIABLE_TITLE, e,
        function(e) {
            e ? a.getVariable(e) ? Blockly.alert(Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1", e.toLowerCase()),
            function() {
                d(e)
            }) : Blockly.Procedures.isNameUsed(e, a) ? Blockly.alert(Blockly.Msg.PROCEDURE_ALREADY_EXISTS.replace("%1", e.toLowerCase()),
            function() {
                d(e)
    ...
    var d = function(e) {
        Blockly.Variables.promptName(Blockly.Msg.RENAME_VARIABLE_TITLE.replace("%1", b.name), e,
        function(e) {
            if (e) {
                var f = a.getVariable(e);
                f && f.type != b.type ? Blockly.alert(Blockly.Msg.VARIABLE_ALREADY_EXISTS_FOR_ANOTHER_TYPE.replace("%1", e.toLowerCase()).replace("%2", f.type),
                function() {
                    d(e)
                }) : Blockly.Procedures.isNameUsed(e, a) ? Blockly.alert(Blockly.Msg.PROCEDURE_ALREADY_EXISTS.replace("%1", e.toLowerCase()),
                function() {
    ...
    return '<field name="VAR" variableType="' + a.type + '" id="' + a.getId() + '">' + a.name + "</field>"
...
}; goog.inherits(Blockly.FieldVariable, Blockly.FieldDropdown); Blockly.FieldVariable.prototype.init = function() {
    ...
    a = a || [""];
    if (0 == a.length) throw a = this.getText(),
...
}; Blockly.Generator.NAME_TYPE = "generated_function"; Blockly.Generator.prototype.INFINITE_LOOP_TRAP = null; Blockly.Generator.prototype.STATEMENT_PREFIX = null; Blockly.Generator.prototype.INDENT = "  "; Blockly.Generator.prototype.COMMENT_WRAP = 60; Blockly.Generator.prototype.ORDER_OVERRIDES = []; Blockly.Generator.prototype.workspaceToCode = function(a) {
    ...
    return a
}; Blockly.Generator.prototype.RESERVED_WORDS_ = ""; Blockly.Generator.prototype.addReservedWords = function(a) {
    ...
    if (!this.definitions_[a]) {
        ...
        for (var c = b.join("\n").replace(this.FUNCTION_NAME_PLACEHOLDER_REGEXP_, c), d; d != c;) d = c,
        c = c.replace(/^((  )*)  /gm, "$1\x00");
        c = c.replace(/\0/g, this.INDENT);
        this.definitions_[a] = c
    }
    return this.functionNames_[a]
}; Blockly.Generator.prototype.init = void 0; Blockly.Generator.prototype.scrub_ = void 0; Blockly.Generator.prototype.finish = void 0; Blockly.Generator.prototype.scrubNakedValue = void 0; Blockly.Names = function(a, b) {
    ...
    return a.toLowerCase() == b.toLowerCase()
}; Blockly.Procedures = {}; Blockly.Procedures.NAME_TYPE = Blockly.PROCEDURE_CATEGORY_NAME; Blockly.Procedures.allProcedures = function(a) {
    a = a.getAllBlocks();
...
}; Blockly.FlyoutButton.MARGIN = 5; Blockly.FlyoutButton.prototype.width = 0; Blockly.FlyoutButton.prototype.height = 0; Blockly.FlyoutButton.prototype.onMouseUpWrapper_ = null; Blockly.FlyoutButton.prototype.createDom = function() {
    var a, b = this.isLabel_ ? "blocklyFlyoutLabel": "blocklyFlyoutButton";
    this.cssClass_ && (b += " " + this.cssClass_);
    this.svgGroup_ = Blockly.utils.createSvgElement("g", {
        "class": b
    },
    this.workspace_.getCanvas());
    this.isLabel_ || (a = Blockly.utils.createSvgElement("rect", {
    ...
    this.svgGroup_),
    c = Blockly.utils.createSvgElement("text", {
    ...
    this.height = 20;
    this.isLabel_ || (a.setAttribute("width", this.width), a.setAttribute("height", this.height));
...
}; Blockly.FlyoutButton.prototype.onMouseUp_ = function(a) { (a = this.targetWorkspace_.getGesture(a)) && a.cancel();
    ...
    for (var b = [], c = [], d = this.permanentlyDisabled_.length = 0, e; e = a[d]; d++) if (e.tagName) {
        ...
        if ("BLOCK" == f) f = Blockly.Xml.domToBlock(e, this.workspace_),
        ...
        c.push(isNaN(e) ? g: e);
        else if ("SEP" == e.tagName.toUpperCase()) e = parseInt(e.getAttribute("gap"), 10),
        !isNaN(e) && 0 < c.length ? c[c.length - 1] = e: c.push(g);
        else if ("BUTTON" == f || "LABEL" == f) e = new Blockly.FlyoutButton(this.workspace_, this.targetWorkspace_, e, "LABEL" == f),
        b.push({
            type: "button",
            button: e
        }),
    ...
    this.layout_(b, c);
    this.listeners_.push(Blockly.bindEventWithChecks_(this.svgBackground_, "mouseover", this,
    function() {
    ...
    d && b.moveBy(0, d);
    var d = b.outputConnection ? Blockly.BlockSvg.TAB_WIDTH: 0,
    e = b.getRelativeToSurfaceXY();
...
}; goog.inherits(Blockly.HorizontalFlyout, Blockly.Flyout); Blockly.HorizontalFlyout.prototype.getMetrics_ = function() {
    ...
    var d = this.MARGIN,
    e = this.RTL ? d: d + Blockly.BlockSvg.TAB_WIDTH;
    this.RTL && (a = a.reverse());
    for (var f = 0,
    g; g = a[f]; f++) if ("block" == g.type) {
        g = g.block;
        for (var h = g.getDescendants(), k = 0; c = h[k]; k++) c.isInFlyout = !0;
        g.render();
        h = g.getSvgRoot();
        k = g.getHeightWidth();
        c = g.outputConnection ? Blockly.BlockSvg.TAB_WIDTH: 0;
        c = this.RTL ? e + k.width: e + c;
        g.moveBy(c, d);
        c = this.createRect_(g, c, d, k, f);
        e += k.width + b[f];
        this.addBlockListeners_(h, g, c)
    } else "button" == g.type && (this.initFlyoutButton_(g.button, e, d), e += g.button.width + b[f])
    ...
    for (var b = 0,
    c = 0,
    d; d = a[c]; c++) b = Math.max(b, d.getHeightWidth().height);
    b += 1.5 * this.MARGIN;
...
}; goog.inherits(Blockly.VerticalFlyout, Blockly.Flyout); Blockly.VerticalFlyout.prototype.getMetrics_ = function() {
    if (!this.isVisible()) return null;
    try {
        var a = this.workspace_.getCanvas().getBBox()
    } catch(e) {
    ...
    this.workspace_.scale = this.targetWorkspace_.scale;
    for (var c = this.MARGIN,
    d = this.RTL ? c: c + Blockly.BlockSvg.TAB_WIDTH, e = 0, f; f = a[e]; e++) if ("block" == f.type) {
        f = f.block;
    ...
    if (this.width_ != b) {
        for (c = 0; d = a[c]; c++) {
            if (this.RTL) {
                var e = d.getRelativeToSurfaceXY().x,
                f = b / this.workspace_.scale - this.MARGIN,
                f = f - Blockly.BlockSvg.TAB_WIDTH;
                d.moveBy(f - e, 0)
            }
    ...
    e = null,
    f = 0,
    g; g = a.childNodes[f]; f++) if (g.tagName) switch (g.tagName.toUpperCase()) {
    case "CATEGORY":
    ...
    for (var b = (a || this.tree_).getChildren(), c = 0, d; d = b[c]; c++) {
        var e = d.getRowElement();
        e && (a = this.hasColours_ ? "8px solid " + (d.hexColour || "#ddd") : "none", this.workspace_.RTL ? e.style.borderRight = a: e.style.borderLeft = a);
        this.addColour_(d)
    }
    ...
    if (goog.events.BrowserFeature.TOUCH_ENABLED) {
        var a = this.getElement();
        Blockly.bindEventWithChecks_(a, goog.events.EventType.TOUCHSTART, this, this.handleTouchEvent_)
    }
    ...
    this.updateRow()
}; Blockly.Toolbox.TreeNode.prototype.onDoubleClick_ = function(a) {}; Blockly.Toolbox.TreeNode.prototype.onKeyDown = function(a) {
    ...
    a),
    d = Blockly.utils.createSvgElement("defs", {},
    c),
    ...
    a.appendChild(e.createDom("blocklyMainBackground")); ! b.hasCategories && b.languageTree && (c = e.addFlyout_("svg"), Blockly.utils.insertAfter_(c, a));
    e.translate(0, 0);
    ...
    b.readOnly || b.hasScrollbars || e.addChangeListener(function() {
        if (!e.isDragging()) {
            ...
            if (a.contentTop < d || a.contentTop + a.contentHeight > a.viewHeight + d || a.contentLeft < (b.RTL ? a.viewLeft: c) || a.contentLeft + a.contentWidth > (b.RTL ? a.viewWidth: a.viewWidth + c)) for (var k = e.getTopBlocks(!1), l = 0, m; m = k[l]; l++) {
                var n = m.getRelativeToSurfaceXY(),
                p = m.getHeightWidth(),
                q = d + 25 - p.height - n.y;
                0 < q && m.moveBy(0, q);
                q = d + a.viewHeight - 25 - n.y;
                0 > q && m.moveBy(0, q);
                q = 25 + c - n.x - (b.RTL ? 0 : p.width);
                0 < q && m.moveBy(q, 0);
                n = c + a.viewWidth - 25 - n.x + (b.RTL ? p.width: 0);
                0 > n && m.moveBy(n, 0)
            }
        }
    });
    ...
    var b = a.options,
    c = a.getParentSvg();
    Blockly.bindEventWithChecks_(c.parentNode, "contextmenu", null,
    function(a) {
        Blockly.utils.isTargetInput(a) || a.preventDefault()
    });
    c = Blockly.bindEventWithChecks_(window, "resize", null,
    function() {
        Blockly.hideChaff(!0);
        Blockly.svgResize(a)
    });
...
var CLOSURE_DEFINES = {
    "goog.DEBUG": !1
}; Blockly.mainWorkspace = null; Blockly.selected = null; Blockly.draggingConnections_ = []; Blockly.clipboardXml_ = null; Blockly.clipboardSource_ = null; Blockly.cache3dSupported_ = null; Blockly.hueToRgb = function(a) {
    ...
    g = function(a) {
        ...
        for (var g = 0,
        h; h = a[g]; g++) if (!b || Blockly.Touch.shouldHandleEvent(h)) Blockly.Touch.setClientFromTouch(h),
        c ? d.call(c, h) : d(h),
        f = !0
    };
    ...
    l = 0, m; m = Blockly.Touch.TOUCH_MAP[b][l]; l++) a.addEventListener(m, k, !1),
    h.push([a, m, k]);
    return h
    ...
    h = 0, k; k = Blockly.Touch.TOUCH_MAP[b][h]; h++) a.addEventListener(k, g, !1),
    f.push([a, k, g]);
    return f
